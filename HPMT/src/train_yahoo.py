    ...
    def get_examples_Yahoo_train(self, filename, size_limit_per_type):
        ...
        for row in readfile:
            ...
            if len(line)==2: # label_id, text
                ...
                type_index =  int(line[0])
                if type_load_size.get(type_index,0)< size_limit_per_type:   #每一类只取130000样本
                    for i in range(10):
                        hypo_list = type2hypothesis.get(i)     #所有的假设
                        if i == type_index:
                            '''pos pair'''
                            for hypo in hypo_list:
                                guid = "train-"+str(exam_co)   #"train-0"
                                text_a = line[1]
                                text_b = hypo
                            ...
                            '''neg pair'''
                            for hypo in hypo_list:
                                guid = "train-"+str(exam_co)     #"train-12"
                                text_a = line[1]
                                text_b = hypo
...
def convert_examples_to_features(examples, label_list, max_seq_length,
    ...
    label_map = {label : i for i, label in enumerate(label_list)}   #['entainment':0,'not entainment':1]

    premise_2_tokenzed={}   #前提
    hypothesis_2_tokenzed={} #假设
    list_2_tokenizedID = {}
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if tokens_a is None:
            tokens_a = tokenizer.tokenize(example.text_a)    #先分词
            premise_2_tokenzed[example.text_a] = tokens_a    #形成字典

        tokens_b = premise_2_tokenzed.get(example.text_b)
        if tokens_b is None:
            tokens_b = tokenizer.tokenize(example.text_b)    #先分词
            hypothesis_2_tokenzed[example.text_b] = tokens_b    #形成字典
        ...
        tokens = tokens_A+tokens_B     #整个的token
        segment_ids = segment_ids_A+segment_ids_B   #每个token对应的标签


        input_ids_A = list_2_tokenizedID.get(' '.join(tokens_A))
        if input_ids_A is None:
            input_ids_A = tokenizer.convert_tokens_to_ids(tokens_A)  #形成对应的id
            list_2_tokenizedID[' '.join(tokens_A)] = input_ids_A     #文本-id
        ...
        input_ids = input_ids_A + input_ids_B    #对应这个训练数据的文本id
...
def main():
    ...
    if args.local_rank == -1 or args.no_cuda:
        ...
        n_gpu = torch.cuda.device_count()     #可用GPU数量为1
    else:
    ...
    args.train_batch_size = args.train_batch_size // args.gradient_accumulation_steps      #64
    ...
    task_name = args.task_name.lower()    #rte

    if task_name not in processors:
        raise ValueError("Task not found: %s" % (task_name))

    processor = processors[task_name]()     #RteProcessor(),实例化这个类
    ...
    if args.do_train:
        ...
        train_examples, seen_types = processor.get_examples_Yahoo_train('/home/ltf/code/BenchmarkingZeroShot-master/BenchmarkingZeroShot/topic/train_pu_half_v0.txt', 130000) #train_pu_half_v1.txt
        # train_examples是类，seen_classes=[0,2,4,6,8]
        num_train_optimization_steps = int(     #609375
    ...
    #print(cache_dir)  /home/ltf/.cache/torch/transformers/distributed_-1

    pretrain_model_dir = '/home/ltf/code/BenchmarkingZeroShot-master/BenchmarkingZeroShotData/FineTuneOnFEVER' #FineTuneOnCombined'# FineTuneOnMNLI
    ...
    if args.do_train:
        ...
        '''load dev set'''
        eval_examples, eval_label_list, eval_hypo_seen_str_indicator, eval_hypo_2_type_index = processor.get_examples_Yahoo_test('/home/ltf/code/BenchmarkingZeroShot-master/BenchmarkingZeroShot/topic/dev.txt', seen_types)
        ...
        '''load test set'''
        test_examples, test_label_list, test_hypo_seen_str_indicator, test_hypo_2_type_index = processor.get_examples_Yahoo_test('/home/ltf/code/BenchmarkingZeroShot-master/BenchmarkingZeroShot/topic/test.txt', seen_types)
