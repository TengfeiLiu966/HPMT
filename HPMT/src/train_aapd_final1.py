...
#     new_h = torch.cat([new_h[0], new_h[1]], 0)                   #组合的新特征
...
#     return torch.cat([g_section[0],g_section[1]],0),torch.cat([g_sentence[0],g_sentence[1]],0), new_h
#
# def norm_g(g):
...
#         self.DGP.append(DecoupledGraphPooling(381, 768, 0.1))
#         self.DGP.append(DecoupledGraphPooling(191, 768, 0.1))
#         self.DGP.append(DecoupledGraphPooling(95, 768, 0.1))
...
#         token_feature = torch.reshape(token_feature, (-1,3*(self.args.max_seq_length-2), 768))            #batch * word num * feature dim
#         #******************************************************************************************************************************************************
#         #生成mask
#         h_mask = sentence_mask.unsqueeze(-1).expand(self.args.train_batch_size, 3, self.args.max_seq_length-2, self.args.max_seq_length-2)
#         v_mask = sentence_mask.unsqueeze(-2).expand(self.args.train_batch_size, 3, self.args.max_seq_length-2, self.args.max_seq_length-2)
#         sentence_mask_full = torch.tensor([scipy.linalg.block_diag(i[0,:], i[1,:], i[2,:]) for i in (h_mask == v_mask).cpu()],dtype=torch.float).to(self.device)
#
#         attention_mask_1 = torch.tensor(attention_mask.view(-1, 3, self.args.max_seq_length).unsqueeze(-1),dtype = torch.float)
#         attention_mask_2 = torch.matmul(attention_mask_1, attention_mask_1.transpose(2, 3))[:, :, 1:-1, 1:-1]
#         section_mask_full = torch.tensor([scipy.linalg.block_diag(i[0,:], i[1,:], i[2,:]) for i in attention_mask_2.cpu()],dtype=torch.float).to(self.device)   #2*1016*1016
#         #*************************************************************************************************************************************************
#         #解耦图池化模块*******
#         section_mask, sentence_mask, new_att1, new_sec_fea1 = self.DGP[0](token_feature,pooled_feature.view(-1, 3, 768),section_mask_full,sentence_mask_full)
...
#
#         u = torch.cat([torch.max(new_att1, dim=1)[0].unsqueeze(1),
#                        torch.max(new_att2, dim=1)[0].unsqueeze(1),
#                        torch.max(new_att3, dim=1)[0].unsqueeze(1),
#                        torch.max(new_sec_fea1, dim=1)[0].unsqueeze(1),
#                        torch.max(new_sec_fea2, dim=1)[0].unsqueeze(1),
#                        torch.max(new_sec_fea3, dim=1)[0].unsqueeze(1),], dim=1)
#
#         logits = self.s2tSA(u)
...
class ClassifyModel(customizedModule):
    def __init__(self, pretrained_model_name_or_path, args, device, is_lock=False):
        ...
        self.args = args
        self.DGP = nn.ModuleList()
        self.DGP.append(DecoupledGraphPooling(1024, 768, 0.1))
        self.DGP.append(DecoupledGraphPooling(256, 768, 0.1))
        self.DGP.append(DecoupledGraphPooling(64, 768, 0.1))
        self.classifier = nn.Linear(768, self.args.num_labels)
        self.init_mBloSA()
        self.s2tSA = s2tSA(768)
        self.device = device
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, sentence_mask = None, label=None, ):
        ...
        # sentence_mask_full = torch.tensor([scipy.linalg.block_diag(i[0,:],i[1,:],i[2,:]) for i in (h_mask == v_mask).cpu()],dtype=torch.float).to(self.device)
        # sentence_mask_full = (h_mask == v_mask).squeeze(1)
        ...
        attention_mask_2 = torch.matmul(attention_mask_1, attention_mask_1.transpose(2, 3))[:, :, 1:-1, 1:-1]
        # section_mask_full = attention_mask_2.squeeze(1)
        # section_mask_full = torch.tensor([scipy.linalg.block_diag(i[0,:],i[1,:],i[2,:]) for i in attention_mask_2.cpu()],dtype=torch.float).to(self.device)   #2*1016*1016
        ...
        #                # torch.max(new_sec_fea1, dim=1)[0].unsqueeze(1),
        #                # torch.max(new_sec_fea2, dim=1)[0].unsqueeze(1),
        #                torch.max(new_sec_fea1, dim=1)[0].unsqueeze(1),], dim=1)
        u = torch.cat([torch.max(new_att1, dim=1)[0],
                       torch.max(new_att2, dim=1)[0],
                       torch.max(new_att3, dim=1)[0],
                       torch.max(new_sec_fea3, dim=1)[0]], dim=1)
        ...
        u = G * fusion + (1 - G) * torch.max(new_sec_fea3, dim=1)[0]
        # logits = self.s2tSA(u)
        logits = self.classifier(u)
        return logits
# class ClassifyModel(customizedModule):
#     def __init__(self, pretrained_model_name_or_path, args, rea, rea1, rea2, rea3, device, is_lock=False):
#         super(ClassifyModel, self).__init__()
#         self.bert = BertModel.from_pretrained(pretrained_model_name_or_path)
#         self.args = args
...
#         #残差控制模块
#
#         # G = F.sigmoid(self.g_W1(token_feature) + self.g_W2(attention_output_18))
#         # attention_output_18 = G * token_feature + (1 - G) * attention_output_18
#         #图池化
#         section_mask,sentence_mask,new_att = self.pools[0](section_mask_full,sentence_mask_full,attention_output_18,pooled_feature.view(-1, 4, 768))  # 2*14*768
#
#         attention_output_16 = self.layers1(new_att,section_mask,sentence_mask)
#         # G = F.sigmoid(self.g_W1(token_feature) + self.g_W2(attention_output_16))
#         # attention_output_16 = G * token_feature + (1 - G) * attention_output_16
#         section_mask,sentence_mask,new_att = self.pools[1](section_mask,sentence_mask,attention_output_16,pooled_feature.view(-1, 4, 768))
#
#         attention_output_14 = self.layers2(new_att,section_mask,sentence_mask)
#         # G = F.sigmoid(self.g_W1(token_feature) + self.g_W2(attention_output_14))
#         # attention_output_14 = G * token_feature + (1 - G) * attention_output_14
#         section_mask,sentence_mask,new_att = self.pools[2](section_mask,sentence_mask,attention_output_14,pooled_feature.view(-1, 4, 768))
#
#         attention_output_12 = self.layers3(new_att,section_mask,sentence_mask)
#         # G = F.sigmoid(self.g_W1(token_feature) + self.g_W2(attention_output_12))
#         # attention_output_12 = G * token_feature + (1 - G) * attention_output_12
#
