...
def main():
    ...
    if args.local_rank == -1 or args.no_cuda:                                                                #前面的符合
        device = torch.device("cuda" if torch.cuda.is_available() and not args.no_cuda else "cpu")           #是否有CUDA
        n_gpu = torch.cuda.device_count()                                                                    #共有几个GPU设备
    else:
    ...
    if args.do_train:
        ...
        train_examples, seen_types = processor.get_examples_Wikipedia_train('/home/ltf/code/BenchmarkingZeroShot-master/BenchmarkingZeroShotData/tokenized_wiki2categories.txt', 100000) #train_pu_half_v1.txt
    ...
    if args.do_train:
        ...
        '''load dev set'''
        eval_examples, eval_label_list, eval_hypo_seen_str_indicator, eval_hypo_2_type_index = processor.get_examples_emotion_test('/home/ltf/code/BenchmarkingZeroShot-master/BenchmarkingZeroShot/emotion/dev.txt', seen_types)
        ...
        '''load test set'''
        test_examples, test_label_list, test_hypo_seen_str_indicator, test_hypo_2_type_index = processor.get_examples_emotion_test('/home/ltf/code/BenchmarkingZeroShot-master/BenchmarkingZeroShot/emotion/test.txt', seen_types)
