...
from bert_processors.abstract_processor import convert_examples_to_hierarchical_features
...
class BertTrainer(object):
    ...
    def train_epoch(self, train_dataloader):
        for step, batch in enumerate(tqdm(train_dataloader, desc="Training")):
            ...
            input_ids, input_mask, segment_ids, sentence_mask, label_ids, image_feature_1= batch
            logits= self.model(input_ids.view(-1, self.args.max_seq_length),image_feature_1,sentence_mask, segment_ids.view(-1, self.args.max_seq_length), input_mask.view(-1, self.args.max_seq_length))
    ...
    def train(self):
        ...
        image_feature_1 = convert_image_to_features(os.path.join(self.args.data_dir,'MMaterials', 'train_vgg_clip.img'), self.args.image_max_len)
        ...
        padded_image_ids_1 = torch.tensor(image_feature_1, dtype=torch.float)
        label_ids = torch.tensor(unpadded_label_ids, dtype=torch.long)
        assert len(padded_input_ids) == len(padded_input_mask) == len(padded_segment_ids) == len(label_ids) == len(padded_sentence_mask) == len(padded_image_ids_1)

        train_data = TensorDataset(padded_input_ids, padded_input_mask, padded_segment_ids,padded_sentence_mask, label_ids, padded_image_ids_1)
        ...
        for epoch in trange(int(self.args.num_train_epochs), desc="Epoch"):
            ...
            if dev_f1 > self.best_dev_f1:
                ...
                self.best_test_pre = test_precision
                self.best_test_recall = test_recall
                torch.save(self.model, self.snapshot_path)
            else:
                self.unimproved_iters += 1
                tqdm.write(
                    "Early Stopping. Epoch: {}, Best Test Acc: {}, Best Test Pre: {},Best Test Recall: {}, Best Test F1: {}".format(
                        epoch, self.best_test_acc, self.best_test_pre, self.best_test_recall, self.best_test_f1))
